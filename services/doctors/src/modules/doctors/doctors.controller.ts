...
export class DoctorController {
  ...
  setAvailability = async (
    ...
    const availability = await this.service.addAvailability(request.params.id, request.body);
  ...
  getSlots = async (
    ...
    const slots = await this.service.getAvailableSlots(request.params.id, request.query.date);
    reply.send({ slots });
  };
}
