...
describe('processExpirationJob', () => {
  it('cancela la cita si todavía está PENDING al expirar el TTL', async () => {
    ...
    await processExpirationJob(
      { appointmentId: 'apt-1' },
      { findStatusById: vi.fn().mockResolvedValue('PENDING'), stateMachine: { transition }, logger },
    );
  ...
  it('es idempotente: no hace nada si la cita ya avanzó de estado', async () => {
    const transition = vi.fn();
    const logger = buildLogger();

    await processExpirationJob(
      { appointmentId: 'apt-2' },
      { findStatusById: vi.fn().mockResolvedValue('CONFIRMED'), stateMachine: { transition }, logger },
    );

    expect(transition).not.toHaveBeenCalled();
  });

  it('no falla si la cita ya no existe', async () => {
    const transition = vi.fn();
    const logger = buildLogger();

    await processExpirationJob(
      { appointmentId: 'apt-3' },
      { findStatusById: vi.fn().mockResolvedValue(null), stateMachine: { transition }, logger },
    );
