...
export const withTenant = async <T>(
  ...
  if (tenantId === undefined) {
    throw new Error(
      'withTenant() llamado fuera de un TenantContext poblado -- revisar que registerTenantContext (requests) o runWithTenant (consumer) esté activo antes de esta llamada',
    );
  }
