...
describe('AuditLog (integración con DB real, Fase 5 / ADR-013)', () => {
  ...
  it('writeAuditLog() escribe una fila con los campos obligatorios', async () => {
    ...
    await runWithTenant(TENANT_A, () =>
      withTenant(prisma, (tx) => writeAuditLog(tx, 'dead_letter.read', 'dead_letter_entry', resourceId, 'success')),
    );
    ...
    expect(row?.action).toBe('dead_letter.read');
    expect(row?.resourceType).toBe('dead_letter_entry');
